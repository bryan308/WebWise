...
extendedDesc: Websites would be fairly boring if they could only display text. Luckily HTML provides a wide variety of elements for displaying all sorts of different media. The most widely used of these is the image element.
---
...
The `<img />` tag is used to insert images into HTML documents. It is a void element, meaning it does not have a closing tag and requires no content. The `<img />` tag typically includes attributes that specify the image source(src), alternative text(alt), width, height, and more.
</Section>
<Section>
### Lesson overview
